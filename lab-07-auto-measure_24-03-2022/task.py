...
def DisplayVolume(volt_adc):
    level = round(volt_adc/MAX_VOLTAGE*len(leds))
...
try:
    ...
    while True:
        ...
        print(volt_on_capacitor)
        
        if volt_on_capacitor > CUT_OFF_VOLT_PROPORTION*MAX_VOLTAGE:
            charge_capacitor = 0
            GPIO.output(PIN_TROYKA, charge_capacitor)

        if (charge_capacitor == 0) and (volt_on_capacitor < (1 - CUT_OFF_VOLT_PROPORTION)*MAX_VOLTAGE):
            break
